...
public class FHT3DExample
{
	...
	//}
	 ...
	 public static boolean powerOf2Size(int w) {
	        {int i=2; while(i<w) {i = i * 2;} return i==w;}
	    }
	 
	public FHT3DExample ( int w, int h, int d )
	{
		if( DEBUG )  System.out.println( "Asserting that dimensions meet design criteria");
		assert (w == h);
		assert (h == d);
		...
		if( DEBUG )  System.out.println( "Generating sin cos tables");
		makeSinCosTables(d,s,c);
		makeSinCosTables(w,sw,cw);
		makeSinCosTables(h,sh,ch);

		if( DEBUG )  System.out.println( "Create a CLMemory Structure for  c, w, ...");
		...
		if( DEBUG )  System.out.println( "Copying data to the device");
	...
	public synchronized long run(FloatBuffer data, boolean inverse)
	{
		...
		if( DEBUG ) System.out.println("translate for inverse");
		this.inverse = (inverse == true) ? 1 : 0;

		if( DEBUG )  System.out.println("Setting CLBuffer to input data buffer...");
	...
	private synchronized long run() 
	{
		...
		if( DEBUG ) { System.out.println("Writing the input data set to the device...");}
		...
		if( DEBUG ) { System.out.println("Enqueing the kernel h...");}
		...
		if( DEBUG ) { System.out.println("Enqueing the kernel s...");}
		queue.put1DRangeKernel( kernels, 0, globalWorkSize, localWorkSize );

		if( DEBUG ) { System.out.println("Enqueing the kernel f...");}
	...
	public static void main(String[] args)
	{
		...
		if( DEBUG ) { System.out.println("Creating NIO Backed Arrays...");}
		FloatBuffer dataA = ByteBuffer.allocateDirect(d*w*h*4).order(ByteOrder.nativeOrder()).asFloatBuffer();

		if( DEBUG ) { System.out.println("Adding good data to the arrays..."); }
		HelperUtils.injectGoodData( dataA, path, "dataAswapQuadrants_float_512_262144");

		if( DEBUG ) { System.out.println("Starting opencl setup..."); }
		time = System.currentTimeMillis();
		FHT3DExample f = new FHT3DExample(  w,  h,  d );
		time = System.currentTimeMillis() - time;
		if(PROFILINGMSG) System.out.println("OpenCL setup took " + (time) + " ms.");

		if( DEBUG ) { System.out.println("Starting opencl setup..."); }
		time = System.currentTimeMillis();
		f.run(dataA, false);
		time = System.currentTimeMillis() - time;
		if(PROFILINGMSG) System.out.println("OpenCL execution took " + (time) + " ms.");
